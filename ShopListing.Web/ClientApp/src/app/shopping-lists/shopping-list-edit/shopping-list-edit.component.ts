...
export class ShoppingListEditComponent implements OnInit {
  ...
  private currentShoppingList: IShoppingList;
  private originalShoppingList: IShoppingList;
  

  get shoppingList(): IShoppingList {
    return this.currentShoppingList;
  }

  set shoppingList(value: IShoppingList) {
  ...
  ngOnInit(): void {
    this.route.data.subscribe(data => {
      const resolvedData: IShoppingListResolved = data['resolvedData'];
  ...
  saveShoppingList(): void{
    if (this.isValid()) {
      if(this.shoppingList.id === null) {
        ...
        this.shoppingListDataService.updateShoppingList(this.shoppingList).subscribe({
          next: () => this.onSaveComplete(`The updated ${this.shoppingList.name} was saved`),
          error: err => this.errorMessage = err
        });
      }
    } else {
  ...
  validate(): void {
    ...
    if(this.shoppingList.name && this.shoppingList.name.length >=3 &&
